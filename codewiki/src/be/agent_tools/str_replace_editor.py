...
def normalize_view_range(view_range: Optional[List[int] | str]) -> Optional[List[int]]:
    """Accept list or string view_range like '[1, 50]' and normalize to list[int]."""
    if view_range is None:
        return None
    if isinstance(view_range, list):
        return view_range
    if isinstance(view_range, str):
        raw = view_range.strip()
        if not raw:
            return None
        try:
...
class EditTool:
    ...
    def __call__(
        ...
        *,
        ...
        view_range: Optional[List[int] | str] = None,
...
async def str_replace_editor(
    ...
    Args:
        ...
        view_range: Optional parameter of `view` command when `path` points to a file. Accepts `[11, 12]` or string form like `\"[11, 12]\"`.
    ...
    normalized_view_range = normalize_view_range(view_range)
    if view_range is not None and normalized_view_range is None:
        return (
            "Error: invalid `view_range`. Use a two-integer array, "
            "for example [11, 12] or \"[11, 12]\"."
        )
    ...
    tool(
        ...
        path=absolute_path,
        file_text=file_text,
        view_range=normalized_view_range,
        old_str=old_str,
        new_str=new_str,
